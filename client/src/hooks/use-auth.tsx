...
type AuthContextType = {
  ...
  updateProfilePictureMutation: UseMutationResult<SelectUser, Error, {profilePicture: string}>;
};
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const updateProfilePictureMutation = useMutation({
    mutationFn: async ({profilePicture}: {profilePicture: string}) => {
      const res = await apiRequest("POST", "/api/profile/picture", {profilePicture});
      return await res.json();
    },
    onSuccess: (user: SelectUser) => {
      queryClient.setQueryData(["/api/user"], user);
